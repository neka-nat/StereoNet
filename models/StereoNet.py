...
# Embedded file name: /media/lxy/sdd1/ActiveStereoNet/StereoNet_pytorch/models/StereoNet.py
# Compiled at: 2018-12-10 15:51:27
# Size of source mod 2**32: 13112 bytes
...
def convbn(in_channel, out_channel, kernel_size, stride, pad, dilation):
    return nn.Sequential(nn.Conv2d(in_channel,
      ...
      dilation=dilation), nn.BatchNorm2d(out_channel))


def convbn_3d(in_channel, out_channel, kernel_size, stride, pad):
    return nn.Sequential(nn.Conv3d(in_channel,
      out_channel,
      kernel_size=kernel_size,
      padding=pad,
      stride=stride), nn.BatchNorm3d(out_channel))
...
class FeatureExtraction(nn.Module):
    ...
    def __init__(self, k):
        ...
        out_channel = 32
        for _ in range(k):
            self.downsample.append(nn.Conv2d(in_channel,
              out_channel,
              kernel_size=5,
              stride=2,
              padding=2))
        ...
        for _ in range(6):
            self.residual_blocks.append(BasicBlock(32,
              32, stride=1, downsample=None, pad=1, dilation=1))
...
class EdgeAwareRefinement(nn.Module):
    ...
    def __init__(self, in_channel):
        ...
        for di in astrous_list:
            self.residual_astrous_blocks.append(BasicBlock(32,
              32, stride=1, downsample=None, pad=1, dilation=di))
    ...
    def forward(self, low_disparity, corresponding_rgb):
        ...
        twice_disparity = F.interpolate(output,
        ...
        return nn.ReLU(inplace=True)(torch.squeeze((twice_disparity + self.conv2d_out(output)),
          dim=1))
...
# okay decompiling StereoNet.cpython-36.pyc
